...
| elevenlabs_video | ElevenLabs TTS + 5-scene Pillow cards + per-scene Ken-Burns zoompan + concat/mux | MP4 (30–60s) | `<video controls>` |
...
RunState {
  ...
  brief: str | None,              # markdown brief written after synth; preferred chat context
}
...
ELEVENLABS_VOICE_ID=...           # optional — defaults to pihaaotj6DtshjIKnSVl (custom "jam" voice)
...
- **SQLite run store** — persists across restarts. Single table `runs` (see `apps/api/src/store/runs.py`). Light idempotent migrations run on every `_init_db()` boot — `PRAGMA table_info` detects missing columns (e.g. `brief` added post-v1) and issues `ALTER TABLE ADD COLUMN` once. No Alembic; the schema is small enough that this stays readable.
...
- **Tool-wrapper output sizes are tuned for fast demo runs.** Synthesize ≤250w; report_1pg ≤250w; report_5pg ≤900w; competitor_doc ≤500w / max 5 rows. AutoContent brief is capped at 2000 chars and per-output-type guidance is sent verbatim as the `text` field (AutoContent's docs describe `text` as "Instructions or query for content generation" — it's a directive, not a cosmetic title; prior versions wrapped it in a `"Research Output — X."` prefix which was dropped). Audio jobs get `duration: "short"`; video jobs get `format: "explainer"`. Bump these only when an output looks too thin.
...
- **Prompts are user-editable at runtime.** The synth prompt, the chat system prompt (with a `{context}` placeholder), three report writers, and 11 media-guidance directives live in `apps/api/src/store/prompts.py`. Defaults are the source of truth; overrides persist to a JSON file (path from `PROMPTS_PATH`, else `<apps/api>/Config/prompts.json`). Reads are mtime-cached so runs pick up edits with no restart. `llm.synthesize`, `llm.write_report`, `autocontent.generate_autocontent`, and the `/runs/{id}/chat` route all resolve their prompts from the store each call. Atomic writes via `tmp + os.replace`. Schema is forward-compat: missing keys fall back to defaults so adding a new output type later doesn't break old saved files.
- **Chat is grounded in the synthesized brief, not the raw payload.** The post-research chat route prefers `state.brief` (persisted after synth) over `state.research_payload`. The brief is a 250-word markdown document — much better chat context than a truncated JSON blob. The chat system prompt's `{context}` placeholder is substituted with the brief (capped at 8000 chars) or, for pre-migration runs without a stored brief, the legacy payload (capped at 15000 chars). If a user strips `{context}` from their edit, the context is appended after the prompt so the route doesn't break.
- **Parallel research input carries concrete template guidance.** `_TEMPLATE_HINTS` in `apps/api/src/tools/parallel.py` maps each non-custom template slug (market_sizing, competitor_scan, customer_pain, company_deep_dive, product_teardown) to a paragraph of specific direction (what to focus on, what counts as a good source). A shared `_QUALITY_FOOTER` ("prefer primary sources… return comprehensive findings, downstream tools summarise") is appended to every run. `custom` templates skip the template block entirely.
- **ElevenLabs is a fully independent pipeline.** `tools/elevenlabs.py` handles `elevenlabs_audio` (TTS → MP3) and `elevenlabs_video` (TTS + 5-scene slideshow with per-scene Ken-Burns zoom + concat/mux → MP4, 30–60s). Dispatched from `writer.generate_output` via `_ELEVENLABS_TYPES`; does not share state with AutoContent and runs concurrently in the same `asyncio.gather` fan-out. ffmpeg binary ships with `imageio-ffmpeg` (no apt-get needed). Video pipeline: narration cap 600 chars (~40s at 15 chars/sec), 5 scene cards (split from headings/sentences with rotating accent colours), per-scene MP4 via `zoompan d=1` on an upscaled 2560×1440 render so zoom stays crisp, concat demuxer joins the clips, final mux adds the MP3 with `-shortest` and `+faststart`. Audio cap remains 2500 chars. Missing key raises `ElevenLabsKeyMissingError` → artifact errors gracefully; other outputs unaffected. All intermediates use `{artifact_id}__*` names so the artifact route's `{id}.*` resolver never mis-serves them. Overall timeout: 600s. **Note:** ElevenLabs's Veo/Sora/Kling image-to-video is their private Creative Studio API (contact-sales only) — we generate the video ourselves with ffmpeg and use ElevenLabs only for the narration audio.
...
- **Prompt settings page** — gear in `RunSidebar` swaps the main panel for `SettingsPanel.tsx`: editable textareas for the brief-synth prompt, the chat system prompt (with `{context}` placeholder hint + green/amber validator), the three report writers (collapsible), and the 11 media-guidance directives (2-col grid). Per-field Reset + global Reset-everything + Discard + dirty indicator. Backend: `apps/api/src/store/prompts.py` holds defaults + mtime-cached JSON store with atomic writes; `apps/api/src/routes/prompts.py` exposes `GET/PUT /prompts` and `POST /prompts/reset`. Runs pick up edits with no restart. `llm.synthesize`, `llm.write_report`, `autocontent.generate_autocontent`, and `/runs/{id}/chat` all read from the store each call.
- **Chat panel relocation + markdown rendering + enriched context** — `ChatPanel.tsx` moved from the right column into the middle column (under the "Current Run" card), wrapped in a `Card` with a header/badge showing `Research ready · N/M outputs` and fixed at `h-[640px]`. Assistant replies render through the prompt-kit `Markdown` component (installed via `npx shadcn add https://prompt-kit.com/c/markdown.json`, pulls `ui/markdown.tsx` + `ui/code-block.tsx`, adds `marked` + `remark-breaks` + `shiki` + `@tailwindcss/typography` via `@plugin` in `index.css`). Empty state shows a "Chat context loaded" card (brief + done artifact types + still-generating count) and three suggestion chips. Backend chat context (in `routes/chat.py`) is built by `_build_system_content(brief, payload, artifact_block)`: substitutes the user-editable `{context}` placeholder with the synthesized brief (preferred, 8k cap), falling back to the Parallel payload (15k cap) for pre-migration runs. On top of that, `_load_artifact_context` scans `state.artifacts`, lists every artifact by type + status, and inlines the body of text-based outputs — AutoContent text formats (faq, study_guide, timeline, quiz, datatable, text) plus a `{artifact_id}.md` source-markdown sidecar that `orchestrator/writer.py` writes for the three report PDFs pre-render (since PDFs aren't text-extractable without adding a dep). `.md`/`.txt` suffix guard stops binary artifacts being read as text; per-artifact 4k cap, 24k total. Sidecar write is best-effort — any `OSError` logs a warn event and the pipeline continues.
...
- **AutoContent API (verified 2026-04-17):** header is `Authorization: Bearer`. POST `/content/Create`, body `{resources: [{type:"text", content}], outputType, text: <instruction>}`. `text` is documented as "Instructions or query for content generation" — the generator LLM reads it as a directive, so we send the user-editable `media_guidance[output_type]` string verbatim with no framing prefix. Poll `/content/Status/{id}` — `status` is numeric: 0=queued, 10..80=in-progress, 100=done, <0=error. Completion fields: `audio_url`, `video_url`, `image_url`, `briefing_doc_url`, `response_text`, `document_content` depending on outputType.
